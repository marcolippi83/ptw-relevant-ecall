...
for train_index, test_index in skf.split(X, Y):
    ...
    tuned_parameters = [{'kernel': ['rbf'], 'gamma': [1, 5e-1, 1e-1], 'C': [50, 100, 500, 1000]}]
    ...
    y_true = Y_test
    y_pred = predictions
    np.savetxt('pred_svm_rbf_' + str(cont) + ".txt", y_pred)
    np.savetxt('true_svm_rbf_' + str(cont) + ".txt", y_true)
